...
export default function CandidationForm({
	...
	useEffect(() => {
		// Reset / auto-select when posts change or dialog opens
		if (open) {
			if (singleMode) {
	...
	function onSubmit(values: z.infer<typeof schema>) {
		if (disabled) {
			toast.error(t("elections.candidation_error"));
			return;
		}
		setSubmitEnabled(false);
		if (!userId) {
			toast.error(`${t("elections.candidation_error")} (No user id)`);
			setSubmitEnabled(true);
			return;
		}
		...
		if (!chosen) {
			toast.error(`${t("elections.candidation_error")} (Invalid post)`);
			setSubmitEnabled(true);
			return;
		}
		...
		createCandidation.mutate(
			{
				...
				query: { user_id: userId, post_id: chosen.post_id },
			},
			{
				onSuccess: () => {
			...
			posts.map((p) => ({
				label:
					i18n.language === "en"
						? p.name_en || p.name_sv
						: p.name_sv || p.name_en,
				value: String(p.election_post_id), // convert to string
			})),
		[posts, i18n.language],
	);
