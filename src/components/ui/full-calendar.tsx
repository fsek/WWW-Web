...
const CalendarWeekView = () => {
	...
	// Helper: Adjust start and end of day to UTC
	const getUtcDayBounds = (day: Date, timeZone: string) => {
		const start = fromZonedTime(
			startOfDay(toZonedTime(day, timeZone)),
			timeZone,
		);
		const end = fromZonedTime(endOfDay(toZonedTime(day, timeZone)), timeZone);
		return { start, end };
	};

	// Helper: Split events spanning multiple days
	const splitEventsByDay = (event: CalendarEvent, timeZone: string) => {
		const eventStartUtc = event.start;
		const eventEndUtc = event.end;

		const startDayUtc = fromZonedTime(
			startOfDay(toZonedTime(eventStartUtc, timeZone)),
			timeZone,
		);
		const endDayUtc = fromZonedTime(
			endOfDay(toZonedTime(eventEndUtc, timeZone)),
			timeZone,
		);

		const days = [];
		for (let day = startDayUtc; day <= endDayUtc; day = addDays(day, 1)) {
			const { start: dayStart, end: dayEnd } = getUtcDayBounds(day, timeZone);
			const dayEvent = {
				...event,
				start: max([eventStartUtc, dayStart]),
				end: min([eventEndUtc, dayEnd]),
			};
			days.push(dayEvent);
		}
		return days;
	...
	// Prepare events by processing multi-day ones
	const processedEvents = useMemo(() => {
		const timeZone = "Europe/Berlin"; // Replace with dynamic timezone if needed
		return events.flatMap((event) => {
			if (differenceInDays(event.end, event.start) > 0) {
				return splitEventsByDay(event, timeZone);
			}
			return event;
		});
	}, [events]);
				...
				<div className="grid grid-cols-7 flex-1">
					{weekDates.map((hours, i) => {
						const dayEvents = processedEvents.filter(
							(event) =>
								isSameDay(
									toZonedTime(event.start, "Europe/Berlin"),
									hours[0],
								) ||
								isSameDay(toZonedTime(event.end, "Europe/Berlin"), hours[0]),
						);
						...
						return (
							...
							>
								{dayEvents.map((event) => (
									<EventGroup
										key={`${event.start.toString()}-${event.end.toString()}`}
										hour={event.start}
										events={[event]}
									/>
								))}
